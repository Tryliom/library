...
if (isset($_REQUEST['lib_list']) && isset($_REQUEST['update'])) {
	...
	curl_setopt($ch, CURLOPT_URL,$urlhost."library/edit");
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // N'affiche pas le résultat dans la page
...
if (isset($_REQUEST['lib_list']) && isset($_REQUEST['delete'])) {
	...
	curl_setopt($ch, CURLOPT_URL,$urlhost."library/delete/$id");
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	$s = curl_exec ($ch);
	curl_close ($ch);
...
if (isset($_REQUEST['lib_add']) && isset($_REQUEST['add'])) {
	...
	curl_setopt($ch, CURLOPT_URL,$urlhost."library/add");
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // N'affiche pas le résultat dans la page
...
echo "
<div>
<table id='user' cellspacing=15><caption style='font-size:40px;'>Gestionnaire</caption>

<tr>
	<form method='get' action=''><td><input type=hidden name=section value=lib><input id='bookmanager' type='submit' value='Bibliothèques' /></td></form>
	<form method='get' action=''><td><input type=hidden name=section value=book><input id='bookmanager' type='submit' value='Livres' /></td></form>
	<form method='get' action=''><td><input type=hidden name=section value=user><input id='bookmanager' type='submit' value='Utilisateurs' /></td></form>
</tr>
...
if ($_SESSION['list']=="book") {
	$libadmin = -1;
		$choice = "adminlol";
		echo "<br>";
		require_once('page.php');	
		$json_source = file_get_contents($urlhost.'book/get/'.$page.'/-1');
		$jd= json_decode($json_source);
		$h = "<br><table id='list' cellspacing='0'><th>Titre</th><th>Auteur</th><th>Date</th><th>Édition</th><th>Editeur</th><th>Description</th><th>Bibliothèque</th><th>Options</th>";
		$m = "";
		for ($i=0;$i<sizeof($jd);$i++) {
			$bid = $jd[$i]->id;
			$title = $jd[$i]->title;
