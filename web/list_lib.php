...
$jd= json_decode($json_source);
	$h = "<h1>Gestion des librairies</h1><table id='list' cellspacing='10'><th>ID</th><th>Nom</th><th>Adresse</th><th>Options</th><tr>";
	$m = "";
	for ($i=0;$i<sizeof($jd);$i++) {
		...
		$name = $jd[$i]->name;
		$m .= "
		<form name='lib_list' method='post'>
		<td ".setWidth($name).">
		<input id='text' type='text' value='$lid' name='id' disabled/>
		</td>
		<td ".setWidth($name).">
		<input id='text' type='text' name='name' value='$name' />
		</td><td ".setWidth($adress).">
		<input id='text' type='text' name='adress' value='$adress' />
		</td><td> <input style='width:100%;' id='button' type='submit' value='Sauvegarder' name='update' /> <input style='width:100%;' id='button' type='submit' value='Supprimer' name='delete' /> </td>
		</form></tr>";
	}
	$b = "<form name='lib_add' method=post><tr>
	<td>
	...
	<td><input style='width:100%;' id='button' type='submit' value='Ajouter une nouvelle librairie' name='add' /></td></tr></form></table>";
	if ($m==="") 
		echo "<p>Aucunes librairies</p>";
	else
		echo "$h $m $b";
	...
	// Add new lib et new book, laisser des cases libres pour, le bouton complète juste l'action
	// Mettre les infos dans des input de text pour modifier
	function setWidth($s) {
